...
def fetch_nasa_neo_data(target_date: str) -> Dict[str, Any]:
    """
    ...
    data points (name, diameter, and velocity) for the largest asteroid
    approaching on the target date.
    ...
    Returns:
        Dict[str, Any]: A sanitized dictionary containing the asteroid's name,
                        ...
                        Returns an error dictionary if the network fails.
    """
    ...
    try:
        logging.info(f"Tool executed: Fetching NASA data for {target_date}")
        ...
        asteroids = data.get("near_earth_objects", {}).get(target_date, [])

        if not asteroids:
            return {"error": f"No asteroids found for date {target_date}"}

        # For this pipeline, let's grab the first/largest asteroid in the list to analyze
        target_asteroid = asteroids[0]
